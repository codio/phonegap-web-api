...
async function uploadToPhonegap(key, file, user, project, uid, params) {
    ...
    var url = '/apps';
    try {
        const project = await mongo.findProject(user, project, uid)
        const appId = project && project.data && project.data.id ? project.data.id : null;
        ...
        if (appId) {
            ...
            try {
                ...
                log.trace('uploadToPhonegap:del', err);
            }
        }
        ...
        await saveToMongo(data, key, user, project, uid);
    } catch (err) {
        log.error('uploadToPhonegap:error', user, project, uid, err);
        throw messages.convertPhonegapError(err)
    }
}
